...
- `./validate.sh` and `./test.sh`: ad-hoc validation and smoke tests for local workflows.
...
## Testing Guidelines
Tests are JavaScript files under `test/`, typically named `*.test.js`. Run `npm test` to execute the suite. Use `test-secrets.js` only as a fixture and never add real credentials. Add tests for new commands or scanning logic when feasible.
...
Do not commit real secrets. Use `test-secrets.js` for demos. Configuration is exposed under `leakLock.dependencyHandling` in `package.json`—update docs if you add or change settings.
