...
## 2026-08-12 — v0.8.1
### Added
- **Purge a value from history, without going through a finding.** New command **Leak Lock: Purge a value from git history** and a **🧹 Purge a value from history** button in the results section. You give the exact text; it counts the commits and lists the files that contain it, asks once, rewrites every branch and tag, and then *proves* the value is gone — reading history with object replacement disabled — before staging the force-push. It exists because the findings pipeline could not be trusted for this: a row can be ineligible for cleanup, and a row's value can be the scanner's truncated display value, which matches nothing while `git filter-repo` still exits 0. The replacement is repository-wide by design; a credential in two files is one credential, and the confirmation names every file it will touch.
