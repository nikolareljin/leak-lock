...
- **The results table and the JSON export now show the value as the file stores it.** When an engine decodes before reporting, the reported value is what was displayed, exported and rewritten — all three wrong in the same way, and the disagreement only visible when someone opened the file and saw different characters (`%3d%3d` on disk, `==` on screen). A finding an engine flags as decoded is now checked against the blob it came from: if the reported value is not there but one of its encodings is, that encoding becomes the finding's value, the row says so ("shown as stored (BASE64 by the engine)"), and the reported form is kept rather than discarded. One blob read per commit and path, and only for findings an engine flagged — an ordinary scan does no extra work.
- **Cleanups target the value as the repository stores it, not as the scanner reported it.** This is the most damaging defect in the release, because it fails silently and at scale. Engines do not agree on what "the secret" is: Gitleaks reports the bytes it matched in the file, but **TruffleHog decodes before it detects** (base64, UTF-16, escaped and percent forms) and reports what it understood — a token written `…%3d%3d` in the file arrives as `…==` — and Nosey Parker can report the text *around* a match rather than the match. A rewrite rule built from such a value matches no blob, `git filter-repo` and BFG both exit 0 on a rule that matches nothing, and the credential survives a cleanup that reports success. Every subsequent search for the reported value agrees it is not there, while the next scan finds it again, because the engine decodes again. Three changes: rules whose value is not found are retried in the forms the value may be stored as (percent-encoded in either hex case, `encodeURIComponent`, JSON- and HTML-escaped, base64, and the decoded form of an encoded one), and **every** matching form becomes a rule — a repository holding the encoded form in a `.env` and the decoded form in a document has both rewritten; TruffleHog's `DecoderName` is recorded, so a decoded value is excluded from rule-building with the transform named rather than contributing a rule that does nothing; and Nosey Parker's context-snippet and `content_unavailable` fallbacks are marked as not-a-value for the same reason.
