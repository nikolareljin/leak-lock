...
### Fixed
- **Preparing a Cleanup No Longer Fails When the Remote Is Unreachable**: "Prepare BFG/Git-only command" ran `git fetch --prune --tags origin` as a preflight, and a failing fetch — SSO-gated remote, expired token, offline, wrong VPN — aborted the whole thing, so the user got no script at all. That is exactly the situation where a script you can read and run by hand matters most, and a button labelled *Prepare* should not have been the thing that surfaced a network failure. The refresh is now best-effort: the script is always generated, the failure is explained in the panel with actionable guidance instead of a wall of raw git output, and only the **in-panel run** is withheld — the generated script refreshes refs and re-checks for unpushed commits itself as its first two steps, so running it manually remains safe. The safety rule from #45 still holds; it now gates execution rather than script generation.
