...
## 2026-08-17 — v0.9.0
### Added
- **A previously exported report can be read back in, and Leak Lock says which of its findings were actually resolved.** Export has existed for a while; nothing ever opened one again, so the question every cleanup ends on — *did all of that go away?* — could only be answered by scanning again and diffing two JSON files by eye. **📥 Import JSON** in the results view, or **Leak Lock: Import Scan Report and Check What Was Resolved** from the Command Palette, renders the report as its own section with a per-finding status: **Resolved**, **Still present**, or **Unverifiable**. Presence is measured against the repository rather than against a scanner, with a pickaxe across every ref (`git log --all -S`) plus a working-tree search that includes untracked files, so a value that was added and later deleted — the shape of most leaks — is found, and no engine needs to be installed to get an answer.
- **Unverifiable is a status, not a blank.** A report exported with redaction carries no value to search for, and a value an engine decoded out of a blob is not the bytes the blob holds; both are reported as unverifiable, never as resolved. A verification that stops at its bound says so, and a report generated against a different repository is called out before any of its statuses are read. A tick that means "we could not check" would be worse than no tick, which is the same reasoning that put the coverage panel next to a clean scan.
- **A report belongs to one repository, and Leak Lock enforces that.** Exports now record which repository they describe, by root commits and origin URL, and importing one into a different repository is refused rather than annotated: every value that was never there would come back *resolved*, and the report would look cleaner than the repository it actually describes. Identity is deliberately not the path, so a clone at `/srv/build/app` and one at `~/code/app` are recognised as the same repository, while two unrelated repositories at the same path are not. A report exported before this release carries no identity and still imports, with the uncertainty stated. **Compare anyway** exists for the genuine cross-repository case and labels the result for as long as it is on screen.
- **A refused import offers to go where the report belongs.** When the repository a report describes is on this machine, the refusal offers **Switch to `<name>`** rather than sending you off to reopen a folder and start again: Leak Lock points itself at that repository and imports the report there. The switch is only offered when that repository still matches the report, the identity is re-checked afterwards so a path that now holds something else still refuses, and because scan results describe the repository they came from, clearing them is confirmed first. Manual redaction rules survive the switch.
- **What is new since the report, and when it appeared.** With a current scan on screen, the section also lists findings the report does not contain, each with the commit that first introduced the value and its date (`git log --all --reverse -S`). Repeated scans become a history rather than unrelated snapshots.
- **Every check behind a "resolved" reads the real objects.** `git filter-repo` leaves a `refs/replace/<old commit>` entry for each commit it rewrites and git honours those refs everywhere, so a pickaxe run straight after a cleanup — the exact moment a report gets imported — walks the rewritten history, finds nothing, and would call a value resolved while the original object still holds it. All three searches run with object replacement disabled, in the argument list and in the environment, matching what the rewrite and verification paths already do.
- Imported findings are a record of a past scan: they are never cleanup targets, are not selectable, and survive a re-scan, since comparing them against a fresh scan is the point of holding them.
...
### Fixed
- **A value shortened for the results table can no longer become a rewrite rule.** Display truncation appends `...` to long values; a rule built from that searches history for text ending in three literal dots and matches nothing. The full value is what the rewrite has always used, so this was only reachable when the full value was never recorded — those findings are now excluded with a reason instead. Tests pin the whole path: a value containing `%3d%3d`, `&`, `<`, `>`, quotes and a backslash arrives from the engine, is escaped for HTML in the table, and still reaches the rule file byte-identical, because the webview sends back only the finding's index and never any secret text.
- **Every finding is reconciled with the bytes it came from, so the results table, the JSON export and the rewrite rule all show what the file actually contains.** An engine that normalises before reporting — TruffleHog decodes (base64, UTF-16, percent forms) and others may too — made all three wrong in the same way: `%3d%3d` on disk displayed, exported and rewritten as `==`, `%2b` as `+`, with the disagreement visible only to someone who opened the file. At scan time each finding is now compared against its own bytes — the blob at its commit, or the file on disk for a working-tree finding. If the reported value is there, nothing changes. If it is not, but one of its encodings is (percent-encoded in either hex case, `encodeURIComponent`, JSON- or HTML-escaped, base64, or the decoded form of an encoded one), **that form is adopted — read out of the file, never computed**, so nothing is invented; the row is marked *shown as stored*, and the engine's reported form is kept alongside rather than discarded. Deliberately not gated on an engine declaring that it decoded: an engine that normalises without saying so produced the identical defect with nothing to trigger the check. One read per commit and path, cached.
- **The cleanup itself no longer trusts a reported value either.** A rewrite rule built from a value that is not in any blob matches nothing, and `git filter-repo` and BFG both exit 0 on a rule that matches nothing — so the credential survives a cleanup that reports success, every later search for the reported value agrees it is absent, and the next scan finds it again because the engine normalises again. Silent, and it scales with the number of values that travel through a URL, a query string or a JSON document. Each rule is now probed against history in every form the value may be stored as, and **every** matching form becomes a rule: a repository holding the encoded form in a `.env` and the decoded form in a document has both rewritten. Regex rules are left alone — encoding variants of a pattern are meaningless. A value found in no form is reported rather than silently prepared. Nosey Parker's context-snippet and `content_unavailable` fallbacks are marked as not-a-value, because rewriting the text *around* a match would replace the wrong thing wherever it appears.
- **A secret in a commit message is now actually removed.** `--replace-text` rewrites blob contents only, so a value quoted in a commit message — which the history keyword search finds, offers for cleanup, and links to a live commit — was searched for in blobs, reported as matching nothing, and left untouched by every rewrite. Cleanups now pass `--replace-message` alongside `--replace-text` (in the extension and in both generated scripts), and the pre-flight and post-rewrite checks search commit messages as well, reporting which surface a surviving value is on.
- **Failed preparation says one sentence, with the reasoning on demand.** The refusal was a paragraph of guidance in a notification. It is now a single line naming the repository, with **Show details** opening a Leak Lock output channel that holds the full reasoning, and — when a scan covered several repositories — **Prepare there** when the value is located in another one.
- **Cleanup messages identify a rule without revealing it.** The refusal and the "this rule matched nothing" reports named the rule by its text, which put the secret into a notification — screenshotted, pasted into tickets, captured by log collectors — and into the console. They now carry a short digest plus the mode and character count: enough to tell two rules apart and to spot both silent no-ops (a value the scanner shortened, quotes copied along with the text), with no part of the value in it.
- **The cleanup runs in the repository the findings belong to, not the folder that was scanned.** A scan can cover a directory that merely contains a repository, or a repository with others nested inside it — each finding already records its owning repository, which is why its commit link resolves. The rewrite used the scanned path regardless, so on those layouts `git filter-repo` ran against a repository where the value does not exist: it succeeded, changed nothing, and every check afterwards agreed the secret was still in history, because it was — in the other repository. The prepared block now names the repository it targets, the custom-rule preview looks in that same repository, and a selection spanning two repositories is refused with both named rather than silently cleaning one.
- **A cleanup whose rules match nothing is refused, before the rewrite rather than after.** `--replace-text` is a literal substring match and both `git filter-repo` and BFG exit 0 when a rule matches no blob, so a rule built from a value the scanner shortened, from a line with a trailing carriage return, or from text copied with its surrounding quotes was indistinguishable from one that worked — until the secret was still there afterwards. Preparation now counts each rule against history first: if none of them match, nothing is prepared and the message says what to check; if some do not, they are named and the rest still run.
- **A cleanup could report success while one selected value stayed in history.** Two silent paths caused it, and both now speak up. A selected finding that is a dependency, is marked *excluded from cleanup*, or carries no value at all was dropped from the rule list without a word — preparation now names each one and why. And `git filter-repo` (like BFG) exits 0 when a rule matches nothing, so a rule whose text does not match the bytes in the commit — a truncated value, a trailing carriage return, surrounding quotes — looked identical to one that worked; every rule is now re-checked against history after the rewrite, with object replacement disabled, and any that did not apply is reported with the commit its value still lives in.
- **git-filter-repo is a tracked dependency.** Dependencies Setup reports it under *Required for Git-only cleanup*, states which form answered (`git filter-repo` subcommand or PATH launcher) and its version, and installs it with `python3 -m pip install --user git-filter-repo` on request. A snap-packaged build is reported as installed-but-confined rather than ready, because it cannot read a repository outside `$HOME` and the rewrite fails before touching a commit. The install is confirmed by re-probing, so a pip success into a directory this window's `PATH` does not contain is reported as what it is.
